...
#
# Em simulação com Gazebo, TF e tópicos usam /clock. Sem use_sim_time nos nós do CLI
# aparece TF_OLD_DATA e TF incoerente. Por omissão usa-se sim time; robô real:
#   DIAG_USE_WALL_CLOCK=1 bash scripts/diagnose_nav_fleet.sh
...
if [[ "${DIAG_USE_WALL_CLOCK:-0}" == "1" ]]; then
  ...
  ROS_SIM_ARGS=(--ros-args -p use_sim_time:=true)
fi
...
if ros2 topic list 2>&1 | grep -qx '/odom'; then
  ros2 topic echo /odom --once --timeout 5 "${ROS_SIM_ARGS[@]}" 2>&1 || log "[A] falhou ou sem mensagens no tempo."
else
...
if ros2 topic list 2>&1 | grep -qx '/tf' || ros2 topic list 2>&1 | grep -qx '/tf_static'; then
  timeout -s INT 5 ros2 run tf2_ros tf2_echo odom base_footprint "${ROS_SIM_ARGS[@]}" 2>&1 | head -30 || log "[B1] falhou ou timeout."
else
  log "[B1] SKIP: sem /tf"
fi
...
if ros2 topic list 2>&1 | grep -qx '/tf' || ros2 topic list 2>&1 | grep -qx '/tf_static'; then
  timeout -s INT 5 ros2 run tf2_ros tf2_echo map odom "${ROS_SIM_ARGS[@]}" 2>&1 | head -30 || log "[B2] falhou ou timeout."
...
if ros2 topic list 2>&1 | grep -qx '/tf' || ros2 topic list 2>&1 | grep -qx '/tf_static'; then
  timeout -s INT 5 ros2 run tf2_ros tf2_echo map base_footprint "${ROS_SIM_ARGS[@]}" 2>&1 | head -30 || log "[B3] falhou (normal se B2 falhou)."
else
  log "[B3] SKIP: sem /tf"
fi
...
if ros2 topic list 2>&1 | grep -qx '/cmd_vel'; then
  ros2 topic echo /cmd_vel --once --timeout 8 "${ROS_SIM_ARGS[@]}" 2>&1 || log "[C] sem mensagens (esperado se o robô não está a receber comando)."
else
